 ...
 * TexturedTriangle
 * 
 * Draws a simple equilateral triangle of a given texture
 * 
 * @param {Integer} x       | Left bound X
 * @param {Integer} y       | Bottom bound Y
 * @param {Integer} width   | The triangle's base width
 ...
 * 
 ********************************************************************************/
ShapeGen.prototype.texturedTriangle = function (x, y, width, texture, color, UVArray)
{
    for(var i = 0; i < width; i++)
    {
        for(var j = i; j < width - i; j++)
...
ShapeGen.prototype.texturedCircle = function (xc, yc, r, texture, color, UVArray, fill)
{
    ...
    var y = r; 
    var d = 3 - 2 * r; 
    this._drawTexturedCircle(xc, yc, x, y, texture, color, UVArray); 
    while (y >= x) 
    {  
        x++; 
        if (d > 0) 
    ...
    if (fill && r > 0) this.texturedCircle(xc, yc, r - 1, texture, color, UVArray, fill);
};
 ...
 * Private method for generating portions of the circle
 ********************************************************************************/
ShapeGen.prototype._drawTexturedCircle = function (xc, yc, x, y, texture, color, UVArray)
{
    if (xc + x >= 0 && yc + y >= 0 && xc + x < this.tileMap.getWidth() && yc + y < this.tileMap.getHeight())
    {
        var renderable = new SpriteRenderable(texture);
